...
async function ensureCardSet(row: NormalizedExcelZipCardRow) {
  if (!row.set_code) return null;
  ...
  if (existingSet) {
    const updates: Partial<Pick<CardSetRecord, "name" | "era_key">> = {};
    ...
    if (Object.keys(updates).length === 0) return existingSet;
  ...
  const { data: insertedSet, error: insertError } = await supabase
    ...
    .insert({
      ...
      name: row.set_name ?? row.set_code
    })
    .select("id,set_code,name,era_key")
    .single<CardSetRecord>();
  if (insertError) throw new Error(insertError.message);

  if (!row.era_key || insertedSet.era_key === row.era_key) return insertedSet;
...
async function ensureCardPrinting(input: {
  cardId: string;
  row: NormalizedExcelZipCardRow;
}) {
  const set = await ensureCardSet(input.row);
  if (!set) return false;

  let existingPrintingQuery = supabase
    .from("card_printings")
    .select("id")
    .eq("card_id", input.cardId)
    .eq("set_id", set.id);

  existingPrintingQuery = input.row.card_number
    ? existingPrintingQuery.eq("card_number", input.row.card_number)
    : existingPrintingQuery.is("card_number", null);

  const { data: existingPrinting, error: existingError } =
    await existingPrintingQuery.maybeSingle<{ id: string }>();
  if (existingError) throw new Error(existingError.message);
  if (existingPrinting) return false;

  const { error: insertError } = await supabase.from("card_printings").insert({
    card_id: input.cardId,
    set_id: set.id,
    card_number: input.row.card_number,
    rarity: input.row.rarity
  });
  if (insertError) throw new Error(insertError.message);
...
async function ensureCardPrintings(input: {
  cardId: string;
  ...
  for (const row of input.rows) {
    ...
    const key = `${row.set_code}::${row.card_number ?? ""}`;
    if (seen.has(key)) continue;
    seen.add(key);

    const added = await ensureCardPrinting({ cardId: input.cardId, row });
...
async function resolveCardAtExecutionTime(input: {
  ...
  if (sameNameCards.length === 0) {
    if (input.group.matchedCard) {
      throw new Error(
        `プレビュー時に見つかった既存カードが登録直前に見つかりません: ${input.group.name}`
      );
    }
    return null;
  }
...
export async function executeExcelZipCardImport(
  ...
  for (const group of groups) {
    ...
    try {
      ...
      const printingAddedCount = await ensureCardPrintings({
        cardId: card.id,
        rows: group.rows
      });
      result.printingAddedCount += printingAddedCount;
