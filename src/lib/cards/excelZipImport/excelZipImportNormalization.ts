...
const ERA_KEY_ALIASES: Record<string, ExcelZipEraKey> = {
  first: "first",
  "1st": "first",
  hundred: "hundred",
  "100": "hundred",
...
const COLUMN_TRUE_ALIASES: Record<BooleanColumn, Set<string>> = {
  ...
  is_original: new Set(["original", "orica"]),
...
function parseEraKeyValue(
  ...
  const eraKey = ERA_KEY_ALIASES[normalized.toLowerCase()];
  if (eraKey) return eraKey;

  pushIssue(
    issues,
    rowNumber,
    "era_key",
    `era_key must be one of first, hundred, ddd, x, god: ${rawValue}`
  );
  return null;
}
...
export function normalizeExcelZipRow(row: RawRow): {
  ...
  const setCode = normalizeImportText(value("set_code")) || null;
  const setName = normalizeImportText(value("set_name")) || null;
  const eraKey = parseEraKeyValue(row.rowNumber, value("era_key"), issues);
  const cardNumber = normalizeImportText(value("card_number")) || null;
  const rarity = normalizeImportText(value("rarity")) || null;
  ...
  const booleans = {
    ...
    is_original: parseBooleanValue(row.rowNumber, "is_original", value("is_original"), issues),
...
const COMPARABLE_FIELDS: ComparableField[] = [
  ...
  {
    label: "is_original",
    getRowValue: (row) => row.is_original,
    getCardValue: (card) => card.is_original ?? false
  },
...
export function isSameExcelZipCardRecord(
  ...
  return (
    ...
    (card.is_original ?? false) === row.is_original &&
