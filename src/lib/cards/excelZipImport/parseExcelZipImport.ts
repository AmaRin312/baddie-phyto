...
function looksLikeMojibake(message: string) {
  return MOJIBAKE_TOKENS.some((token) => message.includes(token));
}

function getMessageTail(message: string) {
  const colonTail = message.split(":").at(-1)?.trim();
  if (colonTail && colonTail !== message) return colonTail;

  const spaceTail = message.split(/\s+/u).at(-1)?.trim();
  return spaceTail && spaceTail !== message ? spaceTail : "";
}

function cleanImportIssueMessage(
  level: ExcelZipImportIssue["level"],
  column: ExcelZipImportIssue["column"],
  message: string,
  rowNumber: number | null
) {
  if (!looksLikeMojibake(message)) return message;

  const tail = getMessageTail(message);
  const suffix = tail ? `: ${tail}` : "";

  if (column === "header") {
    return level === "warning"
      ? `未対応の列です。今回は無視します${suffix}`
      : `ヘッダーに問題があります。2行目のシステム列名を確認してください${suffix}`;
  }

  if (column === "zip") {
    if (message.includes("cards.xlsx")) {
      return "ZIP直下に cards.xlsx を1つだけ配置してください。";
    }
    if (message.includes("images/")) {
      return "ZIP直下に images/ フォルダを配置してください。";
    }
    if (message.includes("MB")) {
      return `ZIPサイズが大きすぎます。${Math.floor(
        EXCEL_ZIP_IMPORT_LIMITS.maxZipSizeBytes / 1024 / 1024
      )}MB以下にしてください。`;
    }
    if (message.includes(String(EXCEL_ZIP_IMPORT_LIMITS.maxFileCount))) {
      return `ZIP内のファイル数が多すぎます。上限: ${EXCEL_ZIP_IMPORT_LIMITS.maxFileCount}`;
    }
    return level === "warning"
      ? `ZIP内に想定外のファイルまたはフォルダがあります。今回は無視します${suffix}`
      : `ZIPの構成に問題があります${suffix}`;
  }

  if (column === "image") {
    if (message.includes("images/") && !tail) {
      return "images/ 内に画像ファイルがありません。";
    }
    if (message.includes("images") && message.includes("/")) {
      return `images内のサブフォルダは使えません${suffix}`;
    }
    if (message.includes("Excel")) {
      return `Excelから参照されていない画像です${suffix}`;
    }
    return `画像に問題があります${suffix}`;
  }

  if (column === "image_file") {
    if (message.includes("images/")) {
      return `image_file は images/ 直下のファイル名だけを入力してください${suffix}`;
    }
    if (message.includes("image_file")) {
      return `image_file の指定に問題があります${suffix}`;
    }
    return `画像ファイル指定に問題があります${suffix}`;
  }

  if (column === "ability") {
    return `Abilityが見つかりません${suffix}`;
  }

  if (column === "db") {
    return `既存カードとの照合に問題があります${suffix}`;
  }

  if (column === "excel") {
    if (message.includes("cards.xlsx")) {
      return "cards.xlsx は 1行目が説明、2行目がシステム列名、3行目以降がデータの形式にしてください。";
    }
    if (message.includes(String(EXCEL_ZIP_IMPORT_LIMITS.maxRowCount))) {
      return `Excelのデータ行が多すぎます。上限: ${EXCEL_ZIP_IMPORT_LIMITS.maxRowCount}`;
    }
    return "カードデータ行がありません。3行目以降にデータを入力してください。";
  }

  if (column === "group") {
    return rowNumber
      ? `同じカード名ですが、カード基本情報が一致しません。${rowNumber}行目付近を確認してください。`
      : "同じカード名ですが、カード基本情報が一致しません。";
  }

  return `インポート内容に問題があります${suffix}`;
}

// Kept only to avoid risky edits around previously mojibaked source lines.
// User-facing messages are normalized by cleanImportIssueMessageForDisplay().
void cleanImportIssueMessage;

function cleanImportIssueMessageForDisplay(
  level: ExcelZipImportIssue["level"],
  column: ExcelZipImportIssue["column"],
  message: string,
  rowNumber: number | null
  ...
  if (column === "header") {
    return level === "warning"
      ? `Unsupported column. This column will be ignored${suffix}`
      : `Header issue. Check the system column names on row 2${suffix}`;
  }

  if (column === "zip") {
    if (message.includes("cards.xlsx")) {
      return "Place exactly one cards.xlsx file directly under the ZIP root.";
    }
    if (message.includes("images/")) {
      return "Place an images/ folder directly under the ZIP root.";
    }
    if (message.includes("MB")) {
      return `ZIP file is too large. Keep it under ${Math.floor(
        EXCEL_ZIP_IMPORT_LIMITS.maxZipSizeBytes / 1024 / 1024
      )}MB.`;
    }
    if (message.includes(String(EXCEL_ZIP_IMPORT_LIMITS.maxFileCount))) {
      return `Too many files in ZIP. Limit: ${EXCEL_ZIP_IMPORT_LIMITS.maxFileCount}`;
    }
    return level === "warning"
      ? `Unexpected file or folder in ZIP. It will be ignored${suffix}`
      : `ZIP structure issue${suffix}`;
  }

  if (column === "image") {
    if (message.includes("images/") && !tail) {
      return "No image files were found in images/.";
    }
    if (message.includes("images") && message.includes("/")) {
      return `Subfolders are not allowed inside images/${suffix}`;
    }
    if (message.includes("Excel")) {
      return `Image is not referenced from Excel${suffix}`;
    }
    return `Image issue${suffix}`;
  }

  if (column === "image_file") {
    if (message.includes("images/")) {
      return `image_file must contain only a file name directly under images/${suffix}`;
    }
    if (message.includes("image_file")) {
      return `image_file value issue${suffix}`;
    }
    return `Image file reference issue${suffix}`;
  }

  if (column === "ability") {
    return `Ability was not found${suffix}`;
  }

  if (column === "db") {
    return `Existing card matching issue${suffix}`;
  }

  if (column === "excel") {
    if (message.includes("cards.xlsx")) {
      return "cards.xlsx must use row 1 for notes, row 2 for system column names, and row 3+ for data.";
    }
    if (message.includes(String(EXCEL_ZIP_IMPORT_LIMITS.maxRowCount))) {
      return `Too many Excel data rows. Limit: ${EXCEL_ZIP_IMPORT_LIMITS.maxRowCount}`;
    }
    return "No card data rows found. Enter data on row 3 or later.";
  }

  if (column === "group") {
    return rowNumber
      ? `Rows with the same card name do not have matching card data. Check around row ${rowNumber}.`
      : "Rows with the same card name do not have matching card data.";
  }

  return `Import content issue${suffix}`;
}
...
function issue(
  ...
  return {
    level,
    column,
    message: cleanImportIssueMessageForDisplay(level, column, message, rowNumber),
    rowNumber
  };
}
...
function isInvalidImageFileReference(fileName: string) {
...
export async function previewExcelZipCardImport(file: File): Promise<{
  ...
  try {
    ...
    rows.forEach((row) => {
      if (isInvalidImageFileReference(row.image_file)) {
        issues.push(
          issue(
            "error",
            "image_file",
            `image_file は images/ 直下のファイル名だけを入力してください: ${row.image_file}`,
            row.rowNumber
          )
        );
        return;
