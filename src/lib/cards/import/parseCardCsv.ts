...
export function parseCardCsv(csvText: string): {
  ...
  try {
    ...
    const expectedHeaders =
      usesVersionedHeaders &&
      headers.length === CARD_CSV_PRE_ORIGINAL_HEADERS.length &&
      headers.every((header, index) => header.trim() === CARD_CSV_PRE_ORIGINAL_HEADERS[index])
        ? CARD_CSV_PRE_ORIGINAL_HEADERS
        : usesVersionedHeaders
          ? CARD_CSV_HEADERS
          : CARD_CSV_LEGACY_HEADERS;
