...
export function filterDeckCandidateCards(input: {
  ...
  return input.cards
    ...
    .filter((card) => !textKeyword || (card.card_text ?? "").toLowerCase().includes(textKeyword))
    .filter((card) => hasAny(card.worlds, input.filters.worlds))
    .filter(
      (card) => input.filters.cardTypes.length === 0 || input.filters.cardTypes.includes(card.card_type),
    )
