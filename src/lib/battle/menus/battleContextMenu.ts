...
import type { AbilityActionId, BattleCardAbilityMap } from "@/lib/battle/abilities/abilityTypes";
...
const AREA_ZONE_IDS: ReadonlyArray<BattleZoneId> = ["center", "left", "right", "item", "set"];

function compactItems(items: Array<BattleMenuItem | undefined>) {
  return items.filter((item): item is BattleMenuItem => item != null);
}

function moveCardCommand(card: BattleCard, toZone: BattleZoneId): BattleCommand | undefined {
  ...
  const useItem: BattleMenuItem = isFlagCard(input.cardRecord)
    ...
    : input.abilityChildren.length > 0
      ? {
          direction: "W",
          label: "使用",
          children: compactItems([buildUsePlacement(input.card), ...input.abilityChildren]),
        }
...
function buildAreaMenu(input: { card: BattleCard; abilityChildren: BattleMenuItem[] }): BattleMenuItem[] {
  const toggleLabel = input.card.zoneId === "set" ? "上下反転" : "レスト／スタンド";
