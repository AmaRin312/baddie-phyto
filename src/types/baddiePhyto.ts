...
export const DECK_VISIBILITY_OPTIONS: ReadonlyArray<{
  ...
  },
  {
    ...
    label: "共有デッキ",
    description:
      "他ユーザーにも公開される自作デッキです。編集と削除は作成者のみです。",
  },
  {
    value: "default",
    label: "サンプルデッキ",
    description:
      "全ユーザー向けの基礎デッキです。編集と削除は作成者のみです。",
  },
];
...
export function getDeckVisibilityLabel(deckVisibility: DeckVisibility | null | undefined) {
  return (
    DECK_VISIBILITY_OPTIONS.find((option) => option.value === deckVisibility)?.label ?? "個人保存"
  );
}

export function getDeckEraLabel(deckEra: DeckEraKey | null | undefined) {
  return DECK_ERA_OPTIONS.find((option) => option.value === deckEra)?.label ?? "未設定";
}
