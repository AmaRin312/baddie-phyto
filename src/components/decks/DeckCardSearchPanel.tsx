...
import { CARD_TYPE_OPTIONS, getCardTypeLabel, type CardType } from "@/types/baddiePhyto";
...
function SearchSection(props: {
  ...
  return (
    <section className="dm-deck-search-section">
      ...
      {props.open ? <div className="dm-deck-search-section-body">{props.children}</div> : null}
    </section>
  );
}
...
export function DeckCardSearchPanel({
  ...
  const setsByEra = useMemo(() => {
    const map = new Map<string, DeckCardSetOption[]>();
    sets.forEach((set) => {
  ...
  function setArray<K extends keyof DeckCardSearchFilters>(key: K, value: DeckCardSearchFilters[K]) {
  ...
  function toggleEra(era: (typeof DECK_CARD_ERA_OPTIONS)[number]["value"]) {
    const nextEras = toggleValue(filters.eras, era);
    const activeEraSet = new Set(nextEras);
    const visibleSetIds = new Set(sets.filter((set) => activeEraSet.has(set.eraKey)).map((set) => set.id));
    setFilters({
      ...filters,
      eras: nextEras,
  ...
  const chips = [
    ...
    ...filters.attributes.map((attribute) => ({
      ...
      label: DECK_CARD_ATTRIBUTE_OPTIONS.find((option) => option.value === attribute)?.label ?? attribute,
      onRemove: () =>
        setArray(
          "attributes",
          ...
          <div className="dm-deck-search-details-body">
            <label className="dm-deck-search-name">
              ...
              <input
                ...
                onChange={(event) => setFilters({ ...filters, cardText: event.target.value })}
              />
            </label>
            ...
            >
              ...
              {filters.eras.length > 0 ? (
                <div className="dm-deck-search-sets">
                  <h4>収録弾</h4>
                  {DECK_CARD_ERA_OPTIONS.filter((option) => filters.eras.includes(option.value)).map((option) => (
