...
const EMPTY_BATTLE_SELECTION: BattleSelection = {
  zoneId: null,
  instanceIds: []
};

const EMPTY_SOUL_SELECTION: SoulSelection = {
...
export function BattleController() {
  ...
  const loadBattle = useCallback(async () => {
    ...
    if (!roomId) {
      setLoading(false);
      setMessage("Battle room を指定できませんでした。");
      return;
    }
    ...
    try {
      ...
      if (!selfSeat) {
        ...
        setSyncMessage(
          "Realtime の座席が未指定です。URL に ?seat=player1 または ?seat=player2 を付けると同期できます。"
        );
      }
      ...
      if (!savedBattleStateResult.data) {
        ...
        if (createResult.error) {
          setMessage("BattleState の初期保存に失敗しました。");
        }
      }
      ...
      if (selfSeat && !syncedSeats[selfSeat]) {
        ...
        if (result.data) {
          ...
          setSyncMessage("自分側の同期PlayerState保存に失敗しました。");
        }
      }
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "BattleState の生成に失敗しました。"
      );
    }

    setLoading(false);
  }, [deckId, roomId, router, selfSeat]);
  ...
  function handleClearSelection() {
    setSelection(EMPTY_BATTLE_SELECTION);
    setSoulSelection(EMPTY_SOUL_SELECTION);
  }

  function closeBattleActionPopup() {
  ...
  function closeDeckCountPopup() {
    setPendingDeckCountPopup(null);
  }

  function closeBiriKinataPopup() {
    setPendingBiriKinataPopup(null);
  }

  function closeFaceDownSoulPopup() {
    setPendingFaceDownSoulPopup(null);
  }

  function closeHyakuganPopup() {
    setPendingHyakuganPopup(null);
  }

  function clearTransientBattleUi() {
    setDragSelection(null);
    setSoulDragSelection(null);
    setPendingDeckDrop(null);
    closeBattleActionPopup();
    closeContextMenu();
    closeSelectionMode();
    closeDeckCountPopup();
    closeBiriKinataPopup();
    closeFaceDownSoulPopup();
    closeHyakuganPopup();
  }

  function confirmBattleAction(messageText: string) {
  ...
  function persistSelfPlayerState(
    ...
    void saveSyncedPlayerBattleState({
      ...
      if (result.error || !result.data) {
        setRealtimeStatus("error");
        setSyncMessage(
          `${selfSeat} 側の同期保存に失敗しました。再読み込み、または別操作で再試行してください。`
        );
  ...
  function executeCommand(command: BattleCommand) {
    if (!isAllowedSelfSideCommand(command)) {
      setSyncMessage("同期操作では opponent 側の PlayerState を直接更新できません。");
      return;
    }

    if (command.type === "SHUFFLE_DECK") {
      const confirmed = confirmBattleAction(
        "山札をシャッフルします。よろしいですか？"
      );
      if (!confirmed) return;
    }
  ...
  function executeCommands(commands: BattleCommand[]) {
    ...
    if (
      ...
      !confirmBattleAction("山札をシャッフルします。よろしいですか？")
    ) {
      return;
    }
  ...
  function handleResetBattleState() {
    ...
    const confirmed = confirmBattleAction(
      "対戦状態を初期状態へリセットします。現在の進行は失われます。よろしいですか？"
    );
    if (!confirmed) return;
  ...
  function handleDeleteRoomState() {
    if (!roomId) return;
    const confirmed = confirmBattleAction(
      "このルームの保存済みBattleStateを削除します。ページを閉じると戻せません。よろしいですか？"
    );
    if (!confirmed) return;
  ...
  function handleContextPlacement(source: BattlePlacementSource) {
    setSelectionMode({
      ...
      description: "Center / Left / Right / Item / Set から選択できます。",
  ...
  function handleStartHyakuganPlacement(pairInstanceId: string) {
    ...
    setSelectionMode({
      ...
      description: "Center / Left / Right から選択できます。",
  ...
  function handleContextUiAction(item: BattleMenuItem) {
    ...
    if (
      ...
      if (pairCandidates.length === 0) {
        setMessage("同じゾーンにもう片方の半身候補がありません。");
      } else {
        setPendingHyakuganPopup({
          sourceInstanceId: pendingContextMenu.sourceCard.instanceId,
          pairCandidates,
  ...
  function handleBiriKinataTargetSelect(targetInstanceId: string) {
    ...
    if (!roomId || !selfSeat) {
      setMessage("ビリ・キナータの起動Abilityには roomId と seat が必要です。");
      closeBiriKinataPopup();
      return;
    }
    ...
    void createBattleAbilityNotification({
      ...
      if (result.error) {
        setMessage(`Ability実行の登録に失敗しました: ${result.error}`);
      }
    });
    closeBiriKinataPopup();
  }
  ...
  function handleConfirmAbilityNotification() {
    ...
    window.setTimeout(() => {
      if (!didChange) {
        setMessage("Ability実行の確認通知を取得できませんでした。画面は更新されませんでした。");
        return;
      }
      ...
      void updateBattleAbilityNotificationStatus({
        ...
        if (result.error) {
          setMessage(`Ability実行の状態更新に失敗しました: ${result.error}`);
          return;
        }
    ...
    <main className="bf-battle-app" onClick={handleBattleBackgroundClick}>
      ...
      {hasHiddenAbilityNotification && (
        <button
          ...
          aria-label={`未読Ability通知 ${pendingAbilityNotificationCount}件を表示`}
        >
          <span>Ability通知</span>
      ...
      {pendingDeckDrop && (
        ...
        >
          ...
          >
            <h2 id="bf-deck-drop-dialog-title">山札への戻し先を選択</h2>
      ...
      {lookedDeckCards.length > 0 && (
        <DeckBrowserPopup
          ...
          description="カードを選択し、移動先を設定してください。未処理カードはまとめて戻せます。"
          mode="move"
