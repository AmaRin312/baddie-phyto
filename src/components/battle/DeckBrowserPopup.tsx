...
import type { DeckBrowserMode, DeckBrowserRequest } from "@/lib/battle/deckBrowser/deckBrowserTypes";
...
export function DeckBrowserPopup({
  ...
  soulCandidates,
  title = "Deck Browser",
  description = "山札上からカードを確認できます。カードを選び、一括で移動先を設定できます。",
  mode = "move",
  ...
  const [bulkDestination, setBulkDestination] = useState<BattleDestination>("none");
  const [actionsById, setActionsById] = useState<Record<string, DeckBrowserDraftAction>>({});
  ...
  const availableDestinations = getBattleDestinationDefinitions(normalizedAllowedDestinations);
  const pageCount = Math.max(1, Math.ceil(cards.length / DECK_BROWSER_PAGE_SIZE));
  const pageCards = useMemo(
    () => cards.slice(page * DECK_BROWSER_PAGE_SIZE, page * DECK_BROWSER_PAGE_SIZE + DECK_BROWSER_PAGE_SIZE),
    [cards, page],
  );
  ...
  const unspecifiedCount = actions.filter((action) => action.destination === "none").length;
  ...
  function toggleCard(instanceId: string) {
    ...
    if (selectedIds.includes(instanceId)) {
      setSelectedIds((currentIds) => currentIds.filter((id) => id !== instanceId));
      ...
      <div className="bf-deck-browser-grid">
        {pageCards.map((battleCard, index) => {
          ...
          return (
            ...
            >
              <span className="bf-deck-browser-order">{page * DECK_BROWSER_PAGE_SIZE + index + 1}</span>
              <BoardCard
                card={card}
        ...
        {cards.length === 0 ? <p className="bf-deck-browser-empty">表示できるカードがありません。</p> : null}
      </div>

      {canEditDestinations ? (
        <section className="bf-deck-browser-selected" aria-label="選択中カード">
            ...
            <div className="bf-deck-browser-bulk">
              <label>
                ...
                <select
                  ...
                  onChange={(event) => setBulkDestination(event.target.value as BattleDestination)}
                >
              ...
              <button type="button" disabled={selectedCards.length === 0} onClick={applyBulkDestination}>
                選択中カードへ適用
              </button>
            </div>
          </header>
          {selectedCards.length === 0 ? (
            ...
            selectedCards.map((battleCard) => {
              ...
              return (
                <article className="bf-deck-browser-selected-row" key={battleCard.instanceId}>
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateAction(battleCard.instanceId, (current) => ({
                          ...current,
                          destination: event.target.value as BattleDestination,
                        }))
                      }
                    >
                      {availableDestinations.map((destination) => (
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateAction(battleCard.instanceId, (current) => ({
                              ...current,
                              soulTargetInstanceId: event.target.value || undefined,
                            }))
                          }
                        >
                      ...
                      <label>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            updateAction(battleCard.instanceId, (current) => ({
                              ...current,
                              soulVisibility: event.target.value as "public" | "face_down",
                            }))
                          }
                        >
