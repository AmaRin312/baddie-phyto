...
function getFlagDisplayName(flag: FlagWithCardRecord) {
  return flag.name || flag.card?.name || "未設定";
}
    ...
    <AppShell>
      ...
      {loading ? (
        ...
        <div className="dm-app-grid">
          <AppCard
            title="フラッグ編集"
            description="ゲーム開始フラッグの設定と選択候補を編集します。"
          >
            <FlagAdminForm
              key={flag.id}
              cards={cards}
          ...
          >
            <div className="dm-card-text-preview">
              ...
              <span>
                初期値: 手札{flag.initial_hand} / ゲージ{flag.initial_gauge} / ライフ
                {flag.initial_life}
              </span>
              <span>
                フラッグ選択候補: {flag.can_be_selected_as_flag ? "表示" : "非表示"}
              </span>
        ...
        <AppCard title="エラー" description={message || "フラッグが見つかりません。"} />
      )}

      {message && flag ? <p className="dm-form-message">{message}</p> : null}
    </AppShell>
  );
}
