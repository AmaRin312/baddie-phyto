    ...
    <AppShell>
      ...
      {loading ? (
        ...
        <div className="dm-app-grid">
          ...
          <AppCard
            title="カード表示プレビュー"
            description="selected_image_id が無い場合は Default画像、画像が無い場合は HTMLカード表示になります。"
          >
            <CardViewer card={card} images={images} />
            {card.is_active ? (
              <Button variant="danger" loading={deleting} onClick={handleDeactivate}>
                無効化する
              </Button>
            ) : (
          ...
          <AppCard
            title="画像アップロード"
            description="Storage bucket card-images へアップロードし、card_images に登録します。thumbnail_path は現在 image_path と同じ扱いです。"
          >
            <CardImageInput
              value={imageFile}
          ...
          <AppCard
            title="Ability紐付け"
            description="abilities.behavior_key をカードへ紐付けます。Battleではこの紐付けを読み込んで Ability を判定します。"
          >
          ...
          <AppCard
            title="登録画像"
            description="1カードにつき Default画像は1枚だけです。"
          >
        ...
        <AppCard title="エラー" description={message || "カードが見つかりません。"} />
      )}

      {message && card ? <p className="dm-form-message">{message}</p> : null}
    </AppShell>
  );
}
