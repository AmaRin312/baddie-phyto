...
function downloadResultCsv(result: ExcelZipImportResult) {
  ...
  anchor.href = url;
  anchor.download = "baddie_phyto_official_bf_import_result.csv";
  anchor.click();
  URL.revokeObjectURL(url);
}

async function fetchOfficialPackCards(packUrls: string[]) {
  const response = await fetch("/api/cards/import/official-bf", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ packUrls }),
  });

  if (!response.ok) {
    throw new Error(`公式カードリスト API の取得に失敗しました。HTTP ${response.status}`);
  }
...
function buildConfirmText(preview: ExcelZipImportPreview) {
  return [
    "この内容で公式カードリストのカードと画像をインポートしますか？",
    "",
    `カードグループ数: ${preview.cardGroups.length}件`,
...
export default function OfficialBfImportPage() {
  ...
  async function handlePreview() {
    const packUrls = splitInputs(inputText);
    if (packUrls.length === 0) {
      setMessage("公式カードリストのパック URL を1行ずつ入力してください。");
      return;
    }
    ...
    try {
      const response = await fetchOfficialPackCards(packUrls);
  ...
  async function handleImport() {
    ...
    setMessage("公式カードリストからのカード取込が完了しました。");
  }
  ...
  return (
    <AppShell>
      <div className="dm-stack">
        ...
        <AppCard
          title="公式カードリスト URL 取込"
          description="公式カードリストのパック URL を1行ずつ入力して、カード情報と画像をまとめて取り込みます。"
        >
          <div className="dm-stack">
            <textarea
              rows={8}
              ...
              placeholder="https://fc-buddyfight.com/cardlist/cardsearch/?expansion=119&sort=no&view=text"
            ...
            <div className="dm-page-actions">
              ...
              >
                {loading ? "取得中..." : "URL を取得してプレビュー"}
              </Button>
            </div>

            <p className="dm-muted-text">1回の取得対象は最大 5 パックです。</p>
          </div>
        </AppCard>

        <AppCard
          title="取込時の注意"
          description="取得した内容をプレビューで確認してから登録します。"
        >
          <ul>
